...
`MakLock-1.1.1-macos-universal.zip` 支持 macOS 13 及以上版本，并同时支持 Apple Silicon 和 Intel Mac。解压后将 `MakLock.app` 拖入 `/Applications`，再打开即可。
...
1. 从 [Releases](https://github.com/supervking/MakLock-Secure/releases/latest) 下载最新版 ZIP。
2. 将 ZIP 与发布页的 `.sha256` 校验文件进行比对。
3. 把 `MakLock.app` 移到 `/Applications`。
